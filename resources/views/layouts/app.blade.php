            ...
            <nav id="sidebar-menu-items" class="flex-1 px-4 py-6 space-y-1.5 hidden md:block">
                ...
                @if(Auth::user()->role === 'pengelola')
                    ...
                    <a href="{{ route('admin.activities.index') }}" 
                        class="flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition duration-150 {{ Route::is('admin.activities.*') ? 'bg-blue-600 text-white shadow-sm glow-blue' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                    ...
                    <a href="{{ route('admin.profile.edit') }}" 
                        class="flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition duration-150 {{ Route::is('admin.profile.edit') ? 'bg-blue-600 text-white shadow-sm glow-blue' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.343 3.94c.09-.542.56-.94 1.11-.94h1.093c.55 0 1.02.398 1.11.94l.149.894c.07.424.384.764.78.93.098.042.196.09.294.145.396.224.871.251 1.258.072l.852-.394a1.125 1.125 0 0 1 1.37.49l.547.947a1.125 1.125 0 0 1-.26 1.43l-.708.583c-.347.286-.514.73-.489 1.182.004.077.006.155.006.233 0 .078-.002.156-.006.233-.025.452.142.896.489 1.182l.708.583a1.125 1.125 0 0 1 .26 1.43l-.547.947a1.125 1.125 0 0 1-1.37.49l-.852-.394c-.387-.18-.862-.152-1.258.072-.098.054-.196.103-.294.145-.396.166-.71.506-.78.93l-.15.894c-.09.543-.56.94-1.11.94h-1.094c-.55 0-1.019-.398-1.11-.94l-.149-.894c-.07-.424-.384-.764-.78-.93a4.22 4.22 0 0 1-.294-.145c-.396-.224-.871-.251-1.258-.072l-.852.394a1.125 1.125 0 0 1-1.37-.49l-.546-.947a1.125 1.125 0 0 1 .26-1.43l.707-.583c.348-.286.514-.73.49-1.182a4.21 4.21 0 0 1 0-.466c.024-.452-.142-.896-.49-1.182l-.707-.583a1.125 1.125 0 0 1-.26-1.43l.546-.947a1.125 1.125 0 0 1 1.37-.49l.852.394c.387.18.862.152 1.258-.072.098-.054.196-.103.294-.145.396-.166.71-.506.78-.93l.15-.894Z" />
                ...
                @else
                    ...
                    <a href="{{ route('student.activities.index') }}" 
                       class="flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition duration-150 {{ Route::is('student.activities.index') ? 'bg-blue-600 text-white shadow-sm glow-blue' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                    ...
                    <a href="{{ route('student.password.edit') }}" 
                       class="flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition duration-150 {{ Route::is('student.password.edit') ? 'bg-blue-600 text-white shadow-sm glow-blue' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
            ...
            <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 z-30 shadow-sm shrink-0">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative" id="profile-dropdown-wrapper">
                        <button type="button" onclick="toggleProfileDropdown()" class="flex items-center gap-3 focus:outline-none cursor-pointer">
                            <div class="hidden sm:flex flex-col text-right text-xs">
                                ...
                                @else
                                    <span class="text-blue-600 font-bold capitalize text-[10px]">Pengelola Asrama</span>
                                @endif
                            </div>
                            @if(Auth::user()->role === 'mahasiswa')
                                <div class="w-9 h-9 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold text-xs shadow shrink-0 hover:ring-2 hover:ring-blue-300 transition">
                                    {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                                </div>
                            @else
                                <div class="w-9 h-9 bg-blue-50 border border-blue-200 text-blue-600 rounded-lg flex items-center justify-center font-bold text-xs shrink-0 hover:ring-2 hover:ring-blue-200 transition">
                                    {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                                </div>
                            @endif
                        </button>
                        ...
                        <!-- Dropdown Menu -->
                        <div id="profile-dropdown-menu" class="hidden absolute right-0 top-full mt-2 w-64 bg-white rounded-xl border border-slate-200 shadow-lg z-50 overflow-hidden transition-all duration-200 opacity-0 translate-y-1" style="transform-origin: top right;">
                            <!-- User Info -->
                            <div class="px-4 py-3 border-b border-slate-100 bg-slate-50/50">
                                <div class="flex items-center gap-3">
                                    @if(Auth::user()->role === 'mahasiswa')
                                        <div class="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold text-sm shrink-0">
                                            {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                                        </div>
                                    @else
                                        <div class="w-10 h-10 bg-blue-50 border border-blue-200 text-blue-600 rounded-lg flex items-center justify-center font-bold text-sm shrink-0">
                                            {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                                        </div>
                                    @endif
                                    <div class="min-w-0">
                            ...
                            <div class="py-1.5">
                                @if(Auth::user()->role === 'pengelola')
                                    <a href="{{ route('admin.profile.edit') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition font-medium">
                                ...
                                @else
                                    <a href="{{ route('student.password.edit') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition font-medium">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 text-slate-400">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                                        </svg>
                                        Ganti Password
                                ...
                                <form action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-rose-600 hover:bg-rose-50 transition font-semibold">
