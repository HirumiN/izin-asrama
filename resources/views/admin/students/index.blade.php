                            ...
                            <tr class="hover:bg-slate-50/50 transition duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form action="{{ route('admin.students.destroy', $student) }}" method="POST" id="form-delete-{{ $student->id }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" 
                                                onclick="showDeleteStudentModal({{ Js::from($student->user->name) }}, {{ Js::from($student->nim) }}, 'form-delete-{{ $student->id }}')"
                                                class="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 border border-rose-700 text-white rounded-lg text-xs font-bold transition duration-150 cursor-pointer shadow-sm flex items-center gap-1.5" 
...
function showDeleteStudentModal(name, nim, formId) {
    targetFormId = formId;

    document.getElementById('confirm-modal-title').textContent = 'Hapus Mahasiswa';
    document.getElementById('confirm-modal-message').textContent = 'Apakah Anda yakin ingin menghapus mahasiswa ini secara permanen?';

    const detailBox = document.getElementById('confirm-modal-detail');
    detailBox.innerHTML = `
        <div class="text-slate-600 font-medium">Mahasiswa: <strong class="text-slate-900 font-bold">${name}</strong> (NIM: ${nim})</div>
        <div class="text-rose-600 font-semibold mt-1">⚠️ Perhatian: Seluruh riwayat absensi shalat, absensi kegiatan, dan izin keluar asrama mahasiswa ini akan dihapus secara permanen dari sistem dan tidak dapat dikembalikan!</div>
    `;

    const iconBg = document.getElementById('confirm-modal-icon-bg');
    iconBg.className = 'p-3 bg-rose-50 border border-rose-100 rounded-xl text-rose-600 shrink-0';
